...
//                System.out.println(arr2[i][j]);
        ...
        for (int v : arr) { // 첫번째 순서부터 차례대로 호출해서... v에 '개별적'으로 주입.
            v++; // 복사본 개념.
            System.out.println(v);
            // v는 해당 루프(반복) 내에서만 유지되는 변수 -> 새롭게 꺼내서 대입/할당
        }
...
//            i = 0; // 이 블럭 내에서 i가 1개로 계속 유지가 되면서...
        }

        int[] ex = {5, 10, 15, 20, 25};
        for (int v : ex) { // 인덱스에 대한 정보를 주지 X
            System.out.println(v);
        ...
        // 일반적으로는 for-each 더 효율적이거나 간단하거나 선호되지만...
        // index를 가지고 같은 배열내의 값을 호출, 수정되는 상황
